(() => {
    ...
    (() => {
        ...
        function formRating() {
            ...
            function formRatingInit(rating, ratingSize) {
                ...
                for (let index = 0; index < ratingSize; index++) {
                    ...
                    ratingItems += `\n\t\t\t\t<label aria-label="Value Rating" class="rating__item">\n\t\t\t\t\t<input aria-label="Value Rating" class="rating__input" type="radio" name="rating" value="${index + 1}">\n\t\t\t\t</label>`;
        ...
        const inputs = document.querySelectorAll("#brown .brown__input");
        inputs.forEach((input => {
            const label = input.closest("div").querySelector(".brown__label-for-input");
            let valueSelected = false;
            const toggleLabel = () => {
                if (input.value || valueSelected) label.classList.add("focused"); else label.classList.remove("focused");
            };
            input.addEventListener("focus", (() => {
                label.classList.add("focused");
            }));
            input.addEventListener("blur", (() => {
                setTimeout(toggleLabel, 0);
            }));
            input.addEventListener("input", (() => {
                valueSelected = input.value ? true : false;
                toggleLabel();
            }));
            ...
            toggleLabel();
            if (input.hasAttribute("data-datepicker")) {
                input.addEventListener("focus", (() => {
            ...
            if (input.id === "time") {
                ...
                document.getElementById("blockTimeTable");
                const applyButton = document.getElementById("setTimeBtn");
                ...
                const cancelButton = document.getElementById("cancelTimeBtn");
                ...
                input.addEventListener("focus", (() => {
                    if (hourSelect.value || minuteSelect.value || ampmSelect.value) label.classList.add("focused");
                }));
                ...
                ampmSelect.addEventListener("click", (() => {
                    label.classList.add("focused");
                }));
                hourSelect.addEventListener("input", toggleLabel);
                minuteSelect.addEventListener("input", toggleLabel);
                ampmSelect.addEventListener("input", toggleLabel);
            }
        }));
        ...
        function initSliders() {
            if (document.querySelector(".swiper")) new swiper_core_Swiper(".swiper", {
                ...
                navigation: {
                    prevEl: ".btn-grp-pagin__swiper-button-prev",
                    nextEl: ".btn-grp-pagin__swiper-button-next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1,
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            const errorOne = document.querySelector(".menu-footer__error--one");
            const errorTwo = document.querySelector(".menu-footer__error--two");
            ...
            btnMailSend.addEventListener("click", (function(event) {
                event.preventDefault();
                if (emailInput.value.trim() && !emailPattern.test(emailInput.value.trim())) showError(errorTwo); else if (!emailInput.value.trim()) showError(errorOne);
            }));
        }));
        const btnSearch = document.getElementById("btn-search");
        const script_form = document.querySelector(".menu__form-search");
        btnSearch.addEventListener("click", (function() {
            if (script_form.classList.contains("active-form")) {
                script_form.classList.remove("active-form");
                setTimeout((() => {
                ...
                setTimeout((() => {
                    script_form.classList.add("active-form");
                }), 10);
            }
        }));
        document.addEventListener("DOMContentLoaded", (function() {
 ...
 //! Значення для 'itemsToShowOnClick:' - передбачає, яка кількість карток (у рядах) буде відображатись при кожному наступному клиці на кнопку
                        const itemArea = document.querySelectorAll(".product__cell");
 //! Контейнер з карткою
                        const showMoreBtn = document.querySelector(".product__btn-inner");
 //! Кнопка для розгортання
                        const container = document.querySelector(".product__show-more");
 //! Блок зі слайдами
                        if (!container || !showMoreBtn || itemArea.length === 0) return;
 //! Припинити виконання, якщо не буде знайдено елементів для відображення.
 ...
 //! При false буде вимкненно масштабування scale на слабких пристроях. При true буде задіяно scale
                        function initializeItems() {
                itemArea.forEach(((item, index) => {
                    //! Приховати всі картки окрім видимих-початкових. Перша ініціалізація для скасування всіх стилів для карток
                    if (index < itemsShown) {
                ...
                //! Оновлення напису для кнопки після ініціалізації
                                updateButtonText();
            }
            //! Стилі для елементів при розгортанні, згортанні
 ...
 //!Час для появи, збільшення
                                                                item.style.opacity = "1";
                                item.style.transform = enableScale ? "scale(1)" : "none";
                            }), 20); //!Період коли з'являються нові картки. Встановити в межах 'від-10 до-90'. При перевищені показника 90 можливий конфлікт у стилях!
                             else {
                                item.style.opacity = "1";
                                item.style.transform = enableScale ? "scale(1)" : "none";
                            }
                        ...
                        item.style.setProperty("will-change", "opacity, transform");
                        item.style.transition = "opacity 0.6s ease, transform 0.6s ease";
 //! Час для зникнення,збільшення
                                                item.style.opacity = "0";
                        item.style.transform = enableScale ? "scale(0.8)" : "none";
 //! Параметр встановлюєтьсся за бажанням. При зникненні карток дає ефект зменшення
                                                setTimeout((() => {
                            item.style.display = "none";
                        }), 400);
 //!Час зникання карток коли вони їдуй вгору. Не задавати значення більше 600! Можливий конфлікт стилів!!!
                                        }
                }));
                //! Оновлення тексту для кнопки після кожного кліку
                                updateButtonText();
            }
            //! Оновлення тексту для кнопки
                        function updateButtonText() {
                showMoreBtn.textContent = itemsShown >= itemArea.length ? "Collapse" : "View all product";
            }
 ...
 //! Для контролування кліку по кнопці
            //! Обробка кліку по кнопці
                        showMoreBtn.addEventListener("click", (function() {
                if (isAnimating) return;
 //! Вийти, якщо триває анімація
                                isAnimating = true;
 //! Встановити прапор анімації в 'true'
                ...
                if (itemsShown >= itemArea.length) {
                    ...
                    setTimeout((() => {
                        ...
                        isAnimating = false;
 //! Встановити прапор анімації назад в 'false'
                                        }), 0);
 //! Pатримка перед перекиданням до самої гори
                                } else {
 ...
 //! Затримка перед переходом до наступної картки. При встановленні значення менше '50' йде дотбыжання від швидкості
                                }
            }));
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            const timePicker = document.getElementById("blockTimeTable");
            const timeWrapper = document.querySelector(".brown__wrap-field--time");
            const setTimeBtn = document.getElementById("setTimeBtn");
            const cancelTimeBtn = document.getElementById("cancelTimeBtn");
            timePicker.style.display = "none";
            inputField.addEventListener("focus", (function() {
                const rect = inputField.getBoundingClientRect();
                const wrapperRect = timeWrapper.getBoundingClientRect();
                timePicker.style.display = "block";
                timePicker.style.top = `${rect.bottom - wrapperRect.top}px`;
                timePicker.style.left = `${rect.left - wrapperRect.left}px`;
            }));
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            const submitButton = document.getElementById("brown__book-a-table");
            const errorPerson = document.querySelector(".brown__error-input-person");
            const errorDate = document.querySelector(".brown__error-input-date");
            const errorTime = document.querySelector(".brown__error-input-time");
            submitButton.addEventListener("click", (function(event) {
                let isFormValid = true;
                if (!personInput.value) {
                    errorPerson.style.display = "block";
                    isFormValid = false;
                } else errorPerson.style.display = "none";
                if (!dateInput.value) {
            ...
            personInput.addEventListener("input", (function() {
                if (personInput.value) errorPerson.style.display = "none";
            }));
            function monitorInputValue(input, errorElement) {
                let previousValue = input.value;
                setInterval((() => {
                    if (input.value !== previousValue) {
                        previousValue = input.value;
                        if (input.value) errorElement.style.display = "none";
                    }
                }), 500);
            }
            monitorInputValue(dateInput, errorDate);
            monitorInputValue(timeInput, errorTime);
        }));
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            const playButtonContainer = document.querySelector(".video-player__box-double-items");
            const heroBoxVideoImg = document.querySelector(".hero__box-video-img");
            heroIntro.style.transition = "opacity 0.5s ease";
            heroIntroVideo.style.transition = "opacity 0.5s ease";
            function playVideo() {
                heroIntro.style.opacity = "0";
                setTimeout((() => {
                    heroIntro.style.display = "none";
                    heroIntroVideo.style.display = "block";
                    heroIntroVideo.style.opacity = "1";
                    video.play();
            ...
            function stopVideo() {
                video.pause();
                video.currentTime = 0;
                heroIntroVideo.style.opacity = "0";
                setTimeout((() => {
            ...
            function isElementScrolledOutBy10svh(el) {
                const rect = el.getBoundingClientRect();
                return rect.top <= -window.innerHeight * .7;
            }
            ...
            window.addEventListener("scroll", (function() {
                if (isElementScrolledOutBy10svh(heroBoxVideoImg) && !video.paused) stopVideo();
            }));
        }));
        window["FLS"] = false;
        menuInit();
